...
- The current implementation uses Node.js 26.8.x with ESM and TypeScript 7 under explicit strict settings. TypeScript 7's native `tsc` is the authoritative compiler. Bun is the target production runtime, adopted one process at a time after compatibility and behavior parity are proven; Node remains the rollback runtime during that migration.
- The current web process uses Node's `node:http` transport and server-rendered HTML strings, native forms, and a small `fetch`/`EventSource` client. The target web transport is Hono with route composition; Hono JSX may structure server views without React or hydration. React, TanStack Router, TanStack Start, tRPC, and Tailwind are not adopted.
...
- Node.js 26 remains the current implementation and rollback runtime. The modernization target is Hono for web transport and Bun for production execution, but each process must retain behavior parity and a tested rollback point before switching. TypeScript 7's native compiler remains authoritative, and pnpm plus Node Playwright remain development and validation tools.
