...
- Production web, collector, migration, and backup processes use the exact Bun 1.3.14 runtime with ESM. TypeScript 7's native `tsc` remains the authoritative compiler; Node.js 26.8.x remains the development/validation toolchain and independently recoverable rollback baseline.
- The web process uses Bun's fetch listener, Hono route composition, and server-rendered Hono JSX views with native forms and a small `fetch`/`EventSource` client. React, TanStack Router, TanStack Start, tRPC, and Tailwind are not adopted.
- PostgreSQL 18 is the durable authority and the existing `pg` driver remains in use. This decision does not introduce an ORM or replace the database driver.
- pnpm, TypeScript's native `tsc --noEmit`, and the Node-based Playwright toolchain remain the development and validation tools. Dependency changes for the Hono and Bun steps are selected and proven in their respective tickets rather than assumed by this product specification.
...
- Live Timeline delivery treats PostgreSQL as authoritative. A committed transaction emits a PostgreSQL notification containing only an entry ID or cursor; the web process converts it to an SSE hint, and clients query missing rows through the existing HTTP JSON route.
...
- Production images for web, collector, migration, and backup run as non-root. Deployment configuration includes separate Deployments, Services, Ingress, NetworkPolicies, ServiceAccounts, scoped RBAC, resource limits, probes, PostgreSQL storage, a one-shot migration Job, and a disabled-by-default backup CronJob.
...
- HTTP transport tests exercise runtime validation and authorization at the transport boundary, including attempts to call owner-only membership, retention, and log operations without the required Capability.
...
- The executable implementation and test suites follow the documented seams: domain-interface tests, a deterministic collector adapter, a real disposable PostgreSQL integration boundary, a Playwright core-loop test, non-root container smoke tests, and offline manifest validation. Tests exercise observable behavior rather than private framework wiring.
- Production Kubernetes compatibility, real Google OAuth callbacks, Cloudflare Access, Argo CD reconciliation, GHCR publication, backup upload, and restore are not claimed by local tests. Bounded authorized VM/kind evidence is tracked separately; production and other external checks remain explicitly unverified until their authorized environments are supplied.
...
- The repository contains the executable implementation, frozen dependency lockfile, and local acceptance evidence. Production credentials, cloud resources, and external integrations remain outside local implementation; bounded authorized VM and kind evidence is recorded separately and does not establish production compatibility.
...
- Bun 1.3.14 is the accepted production runtime for web, collector, migration, and backup after independent behavior-parity checks. TypeScript 7's native compiler remains authoritative, and pnpm plus Node Playwright remain development and validation tools; the parent Node images and entrypoints remain independently recoverable rollback baselines.
