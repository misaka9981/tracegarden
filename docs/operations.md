...
The web, migration, and backup Dockerfiles use the frozen pnpm lockfile and ARM64-pinned Node.js 26.8 base image; the collector uses the exact ARM64-pinned Bun 1.3.14 base after its process-specific compatibility proof. Runtime stages contain only compiled application JavaScript, migration SQL, and production dependencies; source files, development dependencies, and configuration are omitted. Compose runs web, migration, and backup as the non-root `node` user and the collector as non-root `bun`, all with a read-only root filesystem, `no-new-privileges`, all Linux capabilities dropped, and an explicitly sized `/tmp` tmpfs. The prior Node collector image remains the rollback baseline for that process; runtime changes are adopted only after equivalent behavior evidence passes.
