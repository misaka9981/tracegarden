...
The web, collector, and backup Dockerfiles use the exact ARM64-pinned Bun 1.3.14 base after their process-specific compatibility proofs; migration remains on the ARM64-pinned Node.js 26.8.x base image. Runtime stages contain only compiled application JavaScript, migration SQL, backup tooling, and production dependencies; source files, development dependencies, and configuration are omitted. Compose runs web and collector as the non-root `bun` user, migration as non-root `node`, and the suspended backup CronJob as non-root `bun`, all with a read-only root filesystem, `no-new-privileges`, all Linux capabilities dropped, and an explicitly sized `/tmp` tmpfs. The prior Node web, collector, and backup images remain independent rollback baselines; runtime changes are adopted only after equivalent behavior evidence passes.
