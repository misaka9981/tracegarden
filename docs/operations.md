...
The web and collector Dockerfiles use the exact ARM64-pinned Bun 1.3.14 base after their process-specific compatibility proofs; migration and backup use the frozen pnpm lockfile and ARM64-pinned Node.js 26.8.x base image. Runtime stages contain only compiled application JavaScript, migration SQL, and production dependencies; source files, development dependencies, and configuration are omitted. Compose runs web and collector as the non-root `bun` user and migration and backup as non-root `node`, all with a read-only root filesystem, `no-new-privileges`, all Linux capabilities dropped, and an explicitly sized `/tmp` tmpfs. The parent Node web and collector images remain independent rollback baselines; runtime changes are adopted only after equivalent behavior evidence passes.
