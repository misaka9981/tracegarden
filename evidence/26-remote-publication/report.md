...
- The two post-release metadata commits collectively change only this report, its sanitized transcript, and Ticket 26 metadata. A `git diff-tree` comparison from the tagged runtime/publication source was checked before push; no runtime-bearing source was changed after the published commit.
